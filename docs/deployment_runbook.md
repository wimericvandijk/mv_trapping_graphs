...
- Private development repository: `wimericvandijk/mv_trapping_graphs`
...
In the private development repository, the publish workflow expects:
...
11. Open the private development repository `wimericvandijk/mv_trapping_graphs`.
12. Open `Settings`.
13. Open `Secrets and variables`.
14. Open `Actions`.
15. Replace the repository secret `PUBLIC_SITE_DEPLOY_TOKEN` with the new token value.
16. Open the `Actions` tab in the private development repository.
...
3. Check that `PUBLIC_SITE_DEPLOY_TOKEN` was updated in the private development repository rather than the public repository.
